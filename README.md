...
- **サーマル表示の温度**：映像タイプがサーマルのとき、画面全体の最高（赤）・最低（水色）温度とその位置を約 1 秒ごとに映像に重ねて表示します（0x14）。**Alt＋クリック**でその点の温度を測って表示します（0x12、約 15 秒表示）。座標は映像ストリームの画素（1920×1080）基準です。
...
- Camera タブの **Camera time** に、カメラの時計（0x40、5 秒ごとに取得）と GPS 時刻との差を常時表示します（1 秒以上ずれると赤字）。FW 1.0.2 より前は 0x30 / 0x40 に未対応の可能性があり、その場合は「取得できません」と表示します。
